...
<head>
	...
	<title>Handal Yesindo Sejahtera | Data Produk </title>
	...
	<!-- Favicon -->
	<link rel="shortcut icon" type="image/png" href="../logo/head.png" />
	
	<!-- CSS Offline -->
	<link rel="stylesheet" type="text/css" href="css/bootstrap.min.css" />
	<link rel="stylesheet" type="text/css" href="css/font-awesome.min.css" />
	<link rel="stylesheet" type="text/css" href="css/handal2.css" />
...
<body>
	...
	<nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
		<div class="container-fluid">
			<div class="navbar-header">
				...
				<a href="#" class="navbar-brand"><img src="../logo/logo2.png" /></a>
			</div>
	...
	<div id="wrapper">
					...
					<div class="col-lg-12">
						...
						<div class="table-responsive" style="margin-top:10px;">
							<table id="data" class="table table-bordered results" style="margin-bottom: 5%;">
								...
								<tbody>
		<?php
			$sql = "SELECT * FROM product INNER JOIN categories ON categories.cat_id = product.cat_id";
		...
		<?php
							...
							$item = $cat = $brd = $bgImg = $image = $detail = $price = $disc = $stock = $available = "";
							$itemErr = $catErr = $brdErr = $bgImgErr = $detailErr = $priceErr = $discErr = $stockErr = $availableErr = "";
							$A = ""; $B = "";
							...
							if(isset($_POST['save'])){
								...
								$query = mysqli_query($conn,"SELECT * FROM product WHERE item_name='".$_POST['item']."'");
								...
								if(!$error){
									...
									if(mysqli_num_rows($query) > 0){
										...
										$qry = "INSERT INTO product VALUES ('$itemid','$item','$cat','$brd','$bgImgNew','$filename','$detail','$price','$disc','$stock','$available','$regdate')";
		...
		?>
						...
						<form action="?act=add" class="form-horizontal" method="POST" enctype="multipart/form-data">
							...
							<div class="form-group">
								...
								<div class="col-md-10">
									<input type="text" name="item_id" class="form-control" value="<?php echo autoNumber('item_id','product'); ?>">
								</div>
							</div>
							...
							<div class="form-group">
								<label class="col-md-2 control-label">Pilih Tipe Kategori</label>
								<div class="col-md-3">
		...
		<?php
					...
					if(isset($_POST['update'])){
						...
						if(move_uploaded_file($_FILES['bg-img']['tmp_name'],"img/".$bgImgNew)){
							$sql = mysqli_query($conn, "SELECT bgimg FROM product WHERE item_id = '".$id."'");
							$img = mysqli_fetch_array($sql);
									
							if(is_file("img/".$img['bgimg'])){
								unlink("img/".$img['bgimg']);
							}
							mysqli_query($conn,"UPDATE product SET bgimg='$bgImgNew' WHERE item_id='".$id."'");	
						}
								...
								if($error == UPLOAD_ERR_OK){
									...
									if(move_uploaded_file($tmp,"img/".$image)){
										for($i = 0; $i < count($temp); $i++){
											...
											mysqli_query($conn,"UPDATE product SET image='$filename' WHERE item_id='".$id."'");
										}
									}
								}
							}
							
							mysqli_query($conn,"UPDATE product SET item_name='$item', cat_id='$cat', brd_id='$brd', detail='$detail', price='$price', discount='$disc', stock='$stock', available ='$available' WHERE item_id='".$id."'");
							header('location: product.php');
						}
					}
		...
		?>
						<form action="?act=edit&id=<?php echo $_GET['id'];?>" class="form-horizontal" method="POST" enctype="multipart/form-data">
							...
							<!-- Tipe Kategori dan Subkategori -->
							<div class="form-group">
								<label class="col-md-2 control-label">Tipe Kategori</label>
								<div class="col-md-3">
		...
		case "delete":
			if(isset($_GET['id'])){
				...
				$sql = mysqli_query($conn, "SELECT bgimg, image FROM product WHERE item_id = '".$id."'");
				$img = mysqli_fetch_array($sql);
