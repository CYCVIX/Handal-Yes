...
<div class="top-products">
	<div class="container">
		<div class="row product" style="margin-top: 10px;">
			...
			$query = "SELECT * FROM product INNER JOIN brands ON brands.brd_id = product.brd_id INNER JOIN categories ON categories.cat_id = product.cat_id WHERE product.available = 'Ada' AND categories.category = 'Inverter' AND brands.brand = 'Shihlin' ORDER by item_id ASC LIMIT $num, $perpage";
			...
			$sql = mysqli_query($conn, "SELECT * FROM product INNER JOIN brands ON brands.brd_id = product.brd_id INNER JOIN categories ON categories.cat_id = product.cat_id WHERE product.available = 'Ada' AND categories.category = 'Inverter' AND brands.brand = 'Shihlin' ORDER by item_id ASC");
